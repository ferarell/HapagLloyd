...
Public Class FletesOnLineForm
    ...
    Dim dtSource, dtInvoiceIssued, dtOnLinePaymentAudit, dtOnLinePayment, dtErrorProcess As New DataTable
    ...
    Private Sub bbiShowAll_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles bbiShowAll.ItemClick
        ...
        dtQuery = ExecuteAccessQuery("SELECT * FROM OnLinePaymentAuditQry", "").Tables(0)
    ...
    Private Sub bbiDataTransfer_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles bbiDataTransfer.ItemClick
        ...
        For a = 0 To OpenFileDialog1.FileNames.Count - 1
            ...
            If OpenFileDialog1.FileNames(a).ToUpper.Contains("XLS") Then
                InsertDataSurce(LoadExcel(OpenFileDialog1.FileNames(a), "{0}").Tables(0))
            End If
        Next
        If dtSource.Rows.Count = 0 Then
            Return
        End If
        Dim dtInvoiceType As DataTable = ExecuteAccessQuery("SELECT CodeInvoiceType FROM InvoiceType WHERE Include=True", "").Tables(0)
        ...
        For r = 0 To dtSource.Rows.Count - 1
            SplashScreenManager.Default.SetWaitFormDescription("Processing (Row: " & (r + 1).ToString & " of " & dtSource.Rows.Count.ToString & ")")
        ...
        If dtOnLinePayment.Rows.Count = 0 Then
            oLogFileGenerate.TextFileUpdate("FLETESONLINE", "La consulta OnLinePaymentQry no retornó datos.")
            Return
        End If
        ...
        For r = 0 To dtOnLinePayment.Rows.Count - 1
            SplashScreenManager.Default.SetWaitFormDescription("Sending to WebService (Row: " & (r + 1).ToString & " of " & dtOnLinePayment.Rows.Count.ToString & ")")
            ...
            Catch ex As Exception
                ...
                oLogFileGenerate.TextFileUpdate("FLETESONLINE", ex.Message)
                SplashScreenManager.CloseForm(False)
            Finally
                InsertAudit(drAudit, My.User.Name, aResult)
            End Try
        Next
        SplashScreenManager.Default.SetWaitFormDescription("Getting the Log Process")
        Dim dtQuery As New DataTable
    ...
    Private Sub InsertDataSurce(dtTemp As DataTable)
        ...
        End If
        If dtSource.Rows.Count = 0 Then
            dtSource = dtTemp.Copy
        Else
            For r = 0 To dtTemp.Rows.Count - 1
                dtSource.Rows.Add(dtTemp.Rows(r).ItemArray)
            Next
        End If
    End Sub
    ...
    Private Sub InsertRowProcess(drSource As DataRow)
        ...
        Try
            InsertIntoAccess("InvoiceIssued", dtInvoiceIssued.Rows(iPos), "")
        Catch ex As Exception
            oLogProcessUpdate.SetDescriptionLogProcess(iLogProcess, iLogProcessItem, ex.Message)
        End Try
    End Sub
    ...
    Private Sub InsertAudit(drSource As DataRow, MailSender As String, aWSResponse As ArrayList)
        ...
        Try
            ...
            If ExecuteAccessQuery("SELECT * FROM OnLinePaymentAudit WHERE " & sCondition, "").Tables(0).Rows.Count = 0 Then
                InsertIntoAccess("OnLinePaymentAudit", dtOnLinePaymentAudit.Rows(iPos), "")
            Else
