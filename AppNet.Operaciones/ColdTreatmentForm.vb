...
Public Class ColdTreatmentForm
    Dim beDataSource As New DevExpress.XtraEditors.ButtonEdit
    Dim dsVendorData As New dsMain
    Dim dtBreak, dtResult, dtEvents, dtProtocol As New DataTable
        ...
        Try
            For i As Integer = 0 To lines.Count - 1
                ...
                If TextContain(Replace(Mid(lines(i), 1, 4), ".", " "), "MonthOfYear") Then
                    C1 = GetReadingDate(lines(i).Substring(0, 12))
                End If
                ...
                If Mid(lines(i), iTime, 5).Contains(":") Then
                    ...
                    If Not bSkip Then
                        ...
                        dtResult.Rows(iPosition).Item("C1") = C1
    ...
    Private Sub TextFileV2(lines As List(Of String))
        ...
        For i As Integer = 1 To lines.Count - 1
            ...
            End If
            C1 = CDate(lines(i).Substring(12, 10)).ToString.Substring(0, 10)
    ...
    Private Sub UpdateVendorReadings()
        ...
        dtQuery = ExecuteAccessQuery("select * from ColdTreatmentReadings where [BOOKING]='" & Booking & "' and [CONTAINER] = '" & ContainerNumber & "'").Tables(0)
        If dtQuery.Rows.Count > 0 Then
            SplashScreenManager.Default.SetWaitFormDescription("Delete Data Readings of Cold Treatment")
            ExecuteAccessNonQuery("delete from ColdTreatmentReadings where [BOOKING]='" & Booking & "' and [CONTAINER] = '" & ContainerNumber & "'")
        End If
        dtQuery.Rows.Clear()
        For Each ctrow As DataRow In dtResult.Rows
    ...
    Private Sub UpdateVendorEvents()
        ...
        dtQuery = ExecuteAccessQuery("select * from ColdTreatmentEvents where [BOOKING]='" & Booking & "' and [CONTAINER] = '" & ContainerNumber & "'").Tables(0)
        If dtQuery.Rows.Count > 0 Then
        ...
        End If
        dtQuery.Rows.Clear()
        For Each ctrow As DataRow In dtEvents.Rows
    ...
    Private Sub LoadVoyageByContainerNumber()
        ...
        dtVoyage = ExecuteAccessQuery("select * from ColdTreatment where [CONTAINER]='" & ContainerNumber & "'").Tables(0)
    ...
    Friend Function LoadProtocol() As Boolean
        ...
        dtProtocol.Rows.Clear()
        If LoadExcel(beDataSource.EditValue, "CT_PROTOCOL$").Tables(0).Select("[SERVICE] = '" & Service & "' AND [PORT2] = '" & teTSP.Text & "' AND " & " [PORT4] = '" & tePOD.Text & "'").Length > 0 Then
            dtProtocol = LoadExcel(beDataSource.EditValue, "CT_PROTOCOL$").Tables(0).Select("[SERVICE] = '" & Service & "' AND [PORT2] = '" & teTSP.Text & "' AND " & " [PORT4] = '" & tePOD.Text & "'").CopyToDataTable
        End If
    ...
    Private Sub DataValidation()
        ...
        For Each brow As DataRow In dtBreak.Rows
            iBrokes += 1
            dtEvents.Rows.Add("Broke at " & brow(0) & Space(1) & brow(1) & " and restart at " & brow(2) & Space(1) & brow(3))
        Next
        ...
        bsiRemarks2.Caption = "The maximum temperature for this container is: " & IIf(iBrokes > 1, My.Settings.MaxTemp.ToString, My.Settings.MinTemp.ToString)
        Refresh()
            ...
            For Each row As DataRow In dtReadings.Rows
                If Not bSwitch Then
                    If (row(2) > MaxTemp Or row(3) > MaxTemp Or row(4) > MaxTemp) Then
                        dtDataTmp.Rows.Add()
                        dtDataTmp.Rows(dtDataTmp.Rows.Count - 1)("date_ini") = Format(row(0), "dd/MM/yyyy")
                        dtDataTmp.Rows(dtDataTmp.Rows.Count - 1)("time_ini") = row(1)
                        bSwitch = True
                    End If
                Else
                    If (row(2) <= MaxTemp And row(3) <= MaxTemp And row(4) <= MaxTemp) Then
                        dtDataTmp.Rows(dtDataTmp.Rows.Count - 1)("date_end") = Format(row(0), "dd/MM/yyyy")
    ...
    Private Sub beDataSource_Properties_ButtonClick(sender As Object, e As DevExpress.XtraEditors.Controls.ButtonPressedEventArgs)
        ...
        If OpenFileDialog2.ShowDialog() = DialogResult.OK Then
            beDataSource.Text = OpenFileDialog2.FileName
        End If
    End Sub
    ...
    Private Sub GetMessageToSend()
        ...
        DateFrom = Format(CTInitialDate, "dd/MM/yyyy HH:mm")
        DateTo = Format(DateAdd(DateInterval.Day, 15, CTInitialDate), "dd/MM/yyyy HH:mm")
        Deadline = Format(DateAdd(DateInterval.Day, 2, DateTo), "dd/MM/yyyy HH:mm")
    ...
    Private Sub DataMasterUpdate()
        ...
        dtQuery = ExecuteAccessQuery("select * from ColdTreatment where " & Condition).Tables(0).Select(Condition).CopyToDataTable
        SetValues = ""
        ...
        If rgPort.SelectedIndex = 0 Then
            ...
            SetValues = SetValues & IIf(SetValues <> "", ", ", "") & "[INIDATE]='" & Format(DateFrom, "dd/MM/yyyy HH:mm") & "'"
        Else
        ...
        If iDays = 0 Then
            ...
            End If
        End If
        SetValues = SetValues & ", [FINDATE]='" & Format(DateAdd(DateInterval.Day, iDays, CTInitialDate), "dd/MM/yyyy HH:mm") & "'"
        SetValues = SetValues & ", [CTDAYS]=" & iDays.ToString
        If UpdateAccess("ColdTreatment", Condition, SetValues) Then
    ...
    Private Sub VoyageValidate()
        ...
        If rgMode.SelectedIndex > 0 And ExecuteAccessQuery("select * from ColdTreatment where BOOKING='" & lueVoyage.GetColumnValue("BOOKING") & "' and CONTAINER='" & ContainerNumber & "' and REMARKS='CT PASSED'").Tables(0).Rows.Count > 0 Then
            DevExpress.XtraEditors.XtraMessageBox.Show("Cold treatment finalized, will not update.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
            bbiSave.Enabled = False
            Return
        End If
