...
Public Class ColdTreatmentWcfFormBAK
    ...
    Private Sub bbiProcesss_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles bbiProcesss.ItemClick
        ...
        dtQuery = oAppService.ExecuteSQL("SELECT VESSEL FROM tck.ColdTreatment (NOLOCK) WHERE CONTAINER = '" & ContainerNumberTmp & "' ORDER BY CreatedDate DESC").Tables(0)
        If dtQuery.Rows.Count > 1 Then
            If DevExpress.XtraEditors.XtraMessageBox.Show("This container already exists on another voyage (" & dtQuery.Rows(0)(0).ToString & "), do you want continue?", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning) = DialogResult.No Then
                Return
            End If
        End If
        Try
    ...
    Friend Function IsFileV4(lines As List(Of String)) As Boolean
        Dim bResult As Boolean = False
        For l = 0 To 5
            If lines(l).Contains("DAIKIN") Then
                bResult = True
            End If
        Next
        Return bResult
    ...
    Private Sub TextFileV4(lines As List(Of String))
        ...
        For i As Integer = 1 To lines.Count - 1
            ...
            If lines(i).ToUpper.Contains("TIME") And lines(i).Contains("USDA") Then
                iTime = IIf(iTime = 1, InStr(lines(i), "TIME") - 1, iTime)
                If lines(i).ToUpper.Contains("USDA#1") Then
                    iUSDA1 = IIf(iUSDA1 = 1, InStr(lines(i), "USDA#1"), iUSDA1)
                    iUSDA2 = IIf(iUSDA2 = 1, InStr(lines(i), "USDA#2"), iUSDA2)
                    iUSDA3 = IIf(iUSDA3 = 1, InStr(lines(i), "USDA#3"), iUSDA3)
                End If
                Continue For
            End If
            If Not IsDate("20" & Mid(lines(i), 8, 8)) Then
                Continue For
            End If
            If lines(i).Substring(iUSDA1, 7).Trim = "" Or lines(i).Substring(iUSDA2, 7).Trim = "" Or lines(i).Substring(iUSDA3, 7).Trim = "" Then
                Continue For
            End If
            If TextContain(lines(i).Substring(iUSDA1, 7).Trim, "OnlyNumbers") And TextContain(lines(i).Substring(iUSDA2, 7).Trim, "OnlyNumbers") And TextContain(lines(i).Substring(iUSDA3, 7).Trim, "OnlyNumbers") Then
                C1 = Format(CDate("20" & Mid(lines(i), 8, 8)), "yyyy-MM-dd")
                C2 = lines(i).Substring(iTime, 5)
                C3 = Replace(lines(i).Substring(iUSDA1, 7), ",", ".").Trim
                C4 = Replace(lines(i).Substring(iUSDA2, 7), ",", ".").Trim
                C5 = Replace(lines(i).Substring(iUSDA3, 7), ",", ".").Trim
            Else
                bSkip = True
            End If
            If Not bSkip Then
    ...
    Private Sub TextFileV0(lines As List(Of String))
        Dim iLine As Integer = 0
        Dim C1, C2, C3, C4, C5 As String
        Dim bSkip As Boolean
        Dim iTime, iUSDA1, iUSDA2, iUSDA3 As Integer
    ...
    Private Sub UpdateVendorReadings()
        Dim dtQuery As New DataTable
        Dim aParams, aResult As New ArrayList
        ...
        Next
        dtResult.TableName = "Readings"
        SplashScreenManager.Default.SetWaitFormDescription("Inserting Data Readings...")
        aResult.AddRange(oAppService.CustomStoredProcedureExecution("tck.upColdTreatmentReadingsByTable_Insert", aParams.ToArray, dtQuery))
    ...
    Private Sub UpdateVendorEvents()
        ...
        SplashScreenManager.Default.SetWaitFormDescription("Insert Data Events...")
        aResult.AddRange(oAppService.CustomStoredProcedureExecution("tck.upColdTreatmentEventsByTable_Insert", aParams.ToArray, dtQuery))
        If aResult(0) = False Then
            Throw New Exception(aResult(1))
        End If
    End Sub
    ...
    Private Sub GridView1_RowCellStyle(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Grid.RowCellStyleEventArgs) Handles GridView1.RowCellStyle
        ...
        If (e.RowHandle >= 0) Then
            ...
            If e.Column.FieldName = "C3" Then 'USDA1
                ...
                If C3 = "" Or C3 > My.Settings.MaxTemp Then
                    e.Appearance.BackColor = Color.Salmon
                    e.Appearance.BackColor2 = Color.SeaShell
                End If
            End If
            If e.Column.FieldName = "C4" Then 'USDA2
                ...
                If C4 = "" Or C4 > My.Settings.MaxTemp Then
                    e.Appearance.BackColor = Color.Salmon
                    e.Appearance.BackColor2 = Color.SeaShell
                End If
            End If
            If e.Column.FieldName = "C5" Then 'USDA3
                ...
                If C5 = "" Or C5 > My.Settings.MaxTemp Then
                    e.Appearance.BackColor = Color.Salmon
                    e.Appearance.BackColor2 = Color.SeaShell
                End If
            End If
