...
Module SharedModule
    ...
    Friend Function InsertIntoAccess(ByRef Table As String, ByVal drValues As DataRow, DBFile As String) As Boolean
    ...
    Friend Function ScheduleVoyageUpdate(row As DataRow) As Boolean
        Dim bResult As Boolean = True
        Try
            If ExecuteAccessQuery("select * from ScheduleVoyage where [DPVOYAGE]='" & row("DPVOYAGE") & "' and [POL]='" & row("POL") & "'", "dbColdTreatment.accdb").Tables(0).Rows.Count > 0 Then
                ExecuteAccessNonQuery("delete from ScheduleVoyage where [DPVOYAGE]='" & row("DPVOYAGE") & "' and [POL]='" & row("POL") & "'", "dbColdTreatment.accdb")
            End If
            InsertIntoAccess("ScheduleVoyage", row, "dbColdTreatment.accdb")
        Catch ex As Exception
            bResult = False
        End Try
        Return bResult
    ...
    Friend Function GetVendorContact(ProcessCode As String, VendorCode As String, PortCode As String) As String
        Dim sResult As String = ""
        Dim dtQuery As New DataTable
        If PortCode.Length = 0 Then
            PortCode = ""
        End If
        dtQuery = ExecuteAccessQuery("SELECT * FROM VendorContact WHERE ProcessCode='" & ProcessCode & "' AND VendorCode='" & VendorCode & "' AND VendorLocation='" & PortCode & "'", "").Tables(0)
        ...
        If dtQuery.Rows.Count > 0 Then
            For r = 0 To dtQuery.Rows.Count - 1
                If IsDBNull(dtQuery.Rows(r)("VendorContactMail")) Then
                    Continue For
                End If
                sResult += IIf(sResult <> "", ";", "") & dtQuery.Rows(r)("VendorContactMail")
            Next
        End If
        Return sResult
    End Function
