...
Public Class BookingCancellation
    ...
    Dim drMailProcess As DataRow = GetMailProcess(ProcessCode)
    ...
    Friend Sub StartProcess(Items As Outlook.MailItem)
        ...
        Try
            ...
            'End If
            If sIdioma = "ES" Then
                oBooking.PuertoOrigen = OnlyLetters(GetRowCellValueByPosition(dtLines, "Desde Hacia Por", 0, 6, ""))
            Else
                oBooking.PuertoOrigen = OnlyLetters(GetRowCellValueByPosition(dtLines, "From To By", 0, 2, ""))
            End If
            If sIdioma = "ES" Then
                oBooking.PuertoDestino = OnlyLetters(GetRowCellValueByPosition(dtLines, "Desde Hacia Por", 0, 8, ""))
            Else
                oBooking.PuertoDestino = OnlyLetters(GetRowCellValueByPosition(dtLines, "From To By", 0, 4, ""))
            End If
            sPort = IIf(oBooking.PuertoOrigen.Contains("PE"), oBooking.PuertoOrigen, IIf(oBooking.PuertoDestino.Contains("PE"), oBooking.PuertoDestino, ""))
        ...
        If Not oBooking.DepositoVacioIdentificacion Is Nothing Then
            ForwardMessage(oBooking.DepositoVacioIdentificacion, oBooking.Numero, sPort)
        End If
    ...
    Private Sub ForwardMessage(sVendorTaxCode As String, sBookNo As String, sPort As String)
        ...
        oForwardMail.HTMLBody += "<p>Peru EQD Ops - peruequipos@hlag.com</p>"
        oForwardMail.HTMLBody += "<p>Atentamente,<br />"
        oForwardMail.HTMLBody += "<strong style=""color: rgb(0, 0, 0);""><span class=""m_7053714618942395714cs8929618a1"">Hapag-Lloyd Peru</span></strong></p></span></span></p>"
        'oForwardMail.To = "Alberto.Garcia@hlag.com;aremonfe@gmail.com"
        If sVendorContact = "" Then
            sVendorContact = GetVendorContactByPort(ProcessCode, sPort)
        End If
        If sVendorContact = "" Then
            sVendorContact = "4biznessolutions@gmail.com"
        End If
        oForwardMail.BCC = sVendorContact
        oForwardMail.CC = Nothing
        'oForwardMail.BCC = Nothing
    ...
    Function GetVendorContactByPort(ProcessCode, sPort) As String
        Dim sResult As String = ""
        Dim dtQuery As New DataTable
        dtQuery = ExecuteAccessQuery("SELECT * FROM VendorContactByPort WHERE ProcessCode='" & ProcessCode & "' AND VendorLocation='" & sPort & "'", "").Tables(0)
        If dtQuery.Rows.Count > 0 Then
